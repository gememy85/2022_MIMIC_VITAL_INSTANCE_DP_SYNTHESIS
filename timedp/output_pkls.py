...
categorical_metric_df = pd.concat([epsilon_df, pd.DataFrame(
    results[0], columns=original.categorical_columns)])
continuous_metric_df = pd.concat([epsilon_df, pd.DataFrame(
    results[1], columns=original.continuous_columns)])
